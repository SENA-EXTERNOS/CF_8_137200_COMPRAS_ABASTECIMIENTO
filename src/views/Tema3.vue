<template lang="pug">
.curso-main-container.pb-3
  BannerInterno
  .container.tarjeta.tarjeta--blanca.p-4.p-md-5.mb-5
    .titulo-principal.color-acento-contenido
      .titulo-principal__numero
        span 3
      h1 Trazabilidad

    .row.justify-content-center.mt-4
      .col-lg-8.sm-8
        .titulo-sexto.color-secundario.mt-4
          b Figura 1. 
            p #[i Trazabilidad de un producto] 

    .row.mt-4     
      .col-lg-6
        p La trazabilidad es todo el conjunto de acciones que permiten a empresas controlar la cadena de suministro de materias primas, insumos o productos mediante identificación, ubicación y trayectoria para su incorporación previa a su proceso productivo o distribución.
        .col-lg-12
          .cajon.color-primario.p-3            
            p Por medio de la misma podemos saber desde la procedencia de la mercancía hasta su distribución mediante las diferentes cadenas de distribución, es por ello que las organizaciones pueden integrar distintos tipos de trazabilidad para gestionar su función de transformar o distribuir bienes aumentando su oportunidad de mejorar eficacia y optimizar procesos y de paso su eficiencia en la logística.
      .col-lg-6
        figure
          img(src="@/assets/curso/temas/Tema2/img6.svg", alt="Texto que describa la imagen")

    .row
      .col-lg-12
        p.mt-5 Si desea ampliar el tema y conocerlo con más detalle, lo invitamos a ver el siguiente video (Ibáñez, 2021):
          a(href='https://www.youtube.com/watch?v=AMIamNCaUAo' target="_blank") #[b   https://www.youtube.com/watch?v=AMIamNCaUAo] 

    .row.justify-content-center.mt-3  
      .col-lg-8
          .cajon.color-primario.p-3            
            p De acuerdo con lo anterior, a continuación, se muestran los componentes de la trazabilidad.


    Separador
    
    #t_3_1
    
    .titulo-segundo.color-acento-contenido
      h2 3.1  Componentes de la trazabilidad 

    p Los procesos de trazabilidad se dan por la combinación de personas, información, máquinas y #[i software,]  la cual genera como propiedad emergente de este sistema el control de actividades y recursos a favor de una organización.

    .row.justify-content-center.mt-4
      .col-lg-8.sm-8
        .titulo-sexto.color-secundario.mt-4
          b Figura 2. 
            p #[i Componentes de la trazabilidad] 

    .row
      .col-lg-12
      figure
          img(src="@/assets/curso/temas/Tema3/img1.svg", alt="Texto que describa la imagen")

    p.mt-4 No obstante, los componentes son solo una parte de la trazabilidad, mire a continuación su importancia.


    Separador
    
    #t_3_2
    
    .titulo-segundo.color-acento-contenido
      h2 3.2   Importancia de la trazabilidad

    .row
      
      .col-lg-10
        p Dado que la trazabilidad permite el registro histórico de un producto, es posible saber en qué proceso y lugar se encuentra, así como, las transformaciones que pueda haber tenido. Por ello, en la #[b Figura 3]  podrá encontrar los beneficios que aporta:
      .col-lg-2
        figure
          img(src="@/assets/curso/temas/Tema3/img2.svg", alt="Texto que describa la imagen")

    .row.justify-content-center.mt-4
      .col-lg-8.sm-8
        .titulo-sexto.color-secundario.mt-4
          b Figura 3. 
            p #[i Importancia de la trazabilidad] 

    .row
      .col-lg-12
        figure
            img(src="@/assets/curso/temas/Tema3/img3.svg", alt="Texto que describa la imagen")

    .row.justify-content-center.mt-4      
        .col-lg-9
          .cajon.color-primario.p-3            
            p Por su parte, los tipos de trazabilidad son los siguientes.

    Separador
    
    #t_3_3
    
    .titulo-segundo.color-acento-contenido
      h2 3.3    Tipos de trazabilidad 

    .row.mt-2.justify-content-center.mt-5
      .col-12
        .bloque-texto-g.color-primario.coloryellow.tarjetaimgcar.p-3.p-sm-4.p-md-5
          .bloque-texto-g__img.carimg(
            :style="{'background-image': `url(${require('@/assets/curso/temas/Tema3/img4.svg')})`}"
          )
          .bloque-texto-g__texto3.p-4
            .col-11.col-lg-12
              p.text-small Los tipos de trazabilidad que se pretendan implementar en las organizaciones dependen estrictamente de lo complicado que sea su proceso productivo o la cadena de distribución que use para satisfacer a sus clientes, por ello, existen tres tipos de trazabilidad.

    .row.justify-content-center.mt-4
      .col-lg-8.sm-8
        .titulo-sexto.color-secundario.mt-4
          b Figura 4. 
            p #[i Tipos de trazabilidad] 

    .row.justify-content-center
      .col-lg-8
        figure
            img(src="@/assets/curso/temas/Tema3/img5.svg", alt="Texto que describa la imagen")
    .row.justify-content-center.mt-4
      .col-lg-8
        p Asimismo, es importante revisar en qué consiste el #[i software]  de la trazabilidad.


    Separador
    
    #t_3_4
    
    .titulo-segundo.color-acento-contenido
      h2 3.4   #[i Software ] de trazabilidad 

    .row
      .col-lg-10
        p Por último, en cuanto al #[i  software,] este debe ser capaz de registrar todo lo que sucede con el producto a lo largo de la cadena de suministro, permitir su localización en cualquier momento, registrar su origen, código, colores, etc., de manera que se pueda compartir esa información con todas las partes para la gestión de la cadena, recopilar y almacenar la información sin riesgo de pérdida, así como estructurar datos para uso estratégico y la toma de decisiones con base en el análisis de esa información.

      .col-lg-2
        figure
              img(src="@/assets/curso/temas/Tema3/img6.svg", alt="Texto que describa la imagen")


      .row.justify-content-center.mt-3  
        .col-lg-8
          figure
                img(src="@/assets/curso/temas/Tema3/img7.svg", alt="Texto que describa la imagen")


      .row.justify-content-center.mt-5    
        .col-lg-7
          .cajon.color-primario.p-3            
            p En adición a lo anterior, la Figura 5 presenta los principales tipos de software usados en sistemas de trazabilidad.


      .row.justify-content-center.mt-4
        .col-lg-8.sm-8
          .titulo-sexto.color-secundario.mt-4
            b Figura 5. 
              p #[i Software de trazabilidad] 


      .row.justify-content-center.mt-3  
        .col-lg-12
          figure
                img(src="@/assets/curso/temas/Tema3/img9.png", alt="Texto que describa la imagen")

       

      .row.justify-content-center.mt-4
        .col-lg-8.sm-8
          .titulo-sexto.color-secundario.mt-4
            b Figura 5. 
              p #[i Software de trazabilidad]

      .row
        ImagenInfografica.color-acento-botones
          template(v-slot:imagen)
            figure
              img(src='@/assets/curso/temas/Tema3/img10.png', alt='Texto que describa la imagen')
              

          .tarjeta.color-primario.p-3(x="8%" y="6.5%" numero="1")
            b Sistema de gestión empresarial o ERP:
              p Es un #[i software]  integrador donde convergen distintas fuentes de información de las áreas de la organización, como ventas, contabilidad, producción, gestión de existencias, recursos humanos, entre otros. Controla toda la compañía.
          
          .tarjeta.color-primario.p-3(x="52%" y="6.5%" numero="2")
            p  #[b Sistemas de gestión de flotas de transporte o TMS ] 
              b #[i (Transport Management System):]
              |  Organiza y planifica las rutas de reparto. Por lo general, en los envíos de comercio electrónico suele encargarse de notificar el seguimiento al cliente. Para una mayor agilidad de los procesos, puede integrarse con el Sistema Globalmente Armonizado para la Clasificación y Etiquetado de Productos Químicos (SGA) para unificar el etiquetado y el intercambio de información entre sistemas. En el caso de Easy WMS de Mecalux, esta unión es posible gracias al módulo  #[i Multi Carrier Shipping Software.] 
          
          .tarjeta.color-primario.p-3(x="8%" y="92.5%" numero="3")
            p  #[b Sistema de gestión del almacén o WMS ] 
              b #[i ((Warehouse Management System):  ):]
              |  Optimiza todos los flujos de materiales que se producen en la bodega. Registra cada movimiento de un producto y notifica al ERP y al resto de sistemas con que se integre la organización.
          
          
          .tarjeta.color-primario.p-3(x="52%" y="92.5%" numero="4")
            b  #[i Supply Chain Event Management System ] 
              b  (SCEM):  
              p  Controla toda la información que tiene relación con los flujos logísticos y notifica posibles novedades. Contribuye a que cada responsable en su proceso cumpla con su rol, para así evitar que la trazabilidad de la información que alimenta el sistema se vea interrumpida y se pierda.


      .row.justify-content-center.mt-4
        .col-lg-10
          p Ya vimos la trazabilidad, ahora veamos cómo las condiciones de recibo podrían generar mayores aportes.









</template>

<script>
export default {
  name: 'Tema3',
  data: () => ({
    // variables de vue
  }),
  mounted() {
    this.$nextTick(() => {
      this.$aosRefresh()
    })
  },
  updated() {
    this.$aosRefresh()
  },
}
</script>

<style lang="sass"></style>
